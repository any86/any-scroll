...
export default class extends AnyTouch {
    ...
    private __setXY(x: number, y: number): [number, number] {
        ...
        if (allow[0]) {
            // 同步
            this.xy[0] = clamp(x, this.__minXY[0] - this.__options.tolerance, this.__options.tolerance);
        }

        if (allow[1]) {
            this.xy[1] = clamp(y, this.__minXY[1] - this.__options.tolerance, this.__options.tolerance);
        }
